...
lazy val cpsLoomAddOn = project
  .in(file("jvm-loom-addon"))
  .dependsOn(cps.jvm)
  .disablePlugins(SitePreviewPlugin)
  .settings(sharedSettings)
  .settings(
...
lazy val cpsLoomTest = project
  .in(file("jvm-loom-tests"))
...
lazy val compilerPlugin = project
  ...
  .settings(
    ...
    libraryDependencies ++= Seq(
      ...
      ("org.scala-js" %% "scalajs-linker" % "1.20.1").cross(CrossVersion.for3Use2_13) % "test",
      ("org.scala-js" %% "scalajs-env-nodejs" % "1.4.0").cross(CrossVersion.for3Use2_13) % "test"
    ),
    crossVersion := CrossVersion.full,
    crossScalaVersions := Seq("3.3.6", "3.7.2", "3.7.3"),
...
lazy val compilerPluginTests = crossProject(JSPlatform, JVMPlatform, NativePlatform)
  ...
  .jsSettings(
    scalaJSUseMainModuleInitializer := true,
    libraryDependencies += ("org.scala-js" %% "scalajs-junit-test-runtime" % "1.20.1" % Test).cross(CrossVersion.for3Use2_13),
    mimaFailOnNoPrevious := false,
    Test / unmanagedSourceDirectories ++= Seq(
