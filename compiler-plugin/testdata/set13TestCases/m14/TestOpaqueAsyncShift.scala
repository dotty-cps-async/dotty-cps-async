...
object Issue99Maybe1 {
  ...
  def isEmpty[A](a: Issue99Maybe1[A]): Boolean = a.isInstanceOf[Absent.type]
...
//@cps.plugin.annotation.CpsDebugLevel(20)
class TestOpaqueAsyncShift {
  ...
  def testShiftedMapOnOpaqueType1() = {
    ...
    val f = async[Future] {
      val a = Issue99Maybe1(42)
      val r = a.map(x => Future.successful(x + 1).await)
  ...
  /*
  //. This will not works, until we will have find way to fix 2-nd type argument list in implicit search.
  ...
  def testOpaqueShiftWithoutTypeParams() = {
    // implicit val debugLevel: cps.macros.flags.DebugLevel = cps.macros.flags.DebugLevel(20)
    val f = async[Future] {
      val a = Issue99MyStringList("a", "b", "c")
      val r = a.map(x => Future.successful(x + "a").await)
      assert(r.head == "aa")
      r
    }
    //FutureCompleter(f)
