...
class DeadlineContext(m: CpsAsyncMonad[FutureWithDeadline], initDeadline: Long) extends CpsTryMonadContext[FutureWithDeadline] {
  ...
  def setDeadline(newDeadline: Long) = {
    this.synchronized {
      if lastTimerTask != null then TestTimer.cancel(lastTimerTask.nn)
  ...
  def addComputation[T](x: => Future[T]): FutureWithDeadline[T] =
    val p = Promise[T]
    timeoutPromise.future.onComplete { _ =>
      if (!p.isCompleted) then p.tryFailure(new TimeoutException())
    }(ExecutionContext.global)
    p.completeWith(x)
    FutureWithDeadline.Computation(p, this)

  def checkDeadline[T](x: => T): T =
  ...
  override val monad: CpsTryMonad[FutureWithDeadline] = new CpsAsyncEffectMonad[FutureWithDeadline] {
    ...
    override def map[A, B](fa: FutureWithDeadline[A])(f: A => B): FutureWithDeadline[B] = {
      addComputation(fa.future.map(x => checkDeadline(f(x))))
    }

    override def flatMap[A, B](fa: FutureWithDeadline[A])(f: A => FutureWithDeadline[B]): FutureWithDeadline[B] = {
      addComputation(m.flatMap(fa)(x => checkDeadline(f(x))).future)
    }
    ...
    override def mapTry[A, B](fa: FutureWithDeadline[A])(f: Try[A] => B): FutureWithDeadline[B] = {
      addComputation(m.mapTry(fa)(x => checkDeadline(f(x))).future)
    }

    override def flatMapTry[A, B](fa: FutureWithDeadline[A])(f: Try[A] => FutureWithDeadline[B]): FutureWithDeadline[B] = {
      addComputation(m.flatMapTry(fa)(x => checkDeadline(f(x))).future)
    }
...
class FutureWithDeadlineCpsMonad(using ExecutionContext)
    extends CpsAsyncMonad[FutureWithDeadline]
    with CpsTryContextMonad[FutureWithDeadline, DeadlineContext] {
  ...
  def map[A, B](fa: FutureWithDeadline[A])(f: A => B): FutureWithDeadline[B] =
    fa match
      case FutureWithDeadline.Computation(pa, ca) =>
        ca.addComputation(pa.future.map(x => ca.checkDeadline(f(x))))
    ...
    fa match
      case FutureWithDeadline.Computation(pa, ca) =>
        ca.addComputation(fa.future.flatMap { a =>
          try ca.checkDeadline(f(a)).future
          catch
            case NonFatal(ex) =>
              Future failed ex
        })
  ...
  def tryValue[A](f: => A): FutureWithDeadline[A] =
    try FutureWithDeadline.Value(Success(f))
