                    ...
                    ComputationBound.advanceDeferredQueueTicks(timeRest, ref.get().isDefined).flatMap { r =>
                      ref.get match
                        ...
                        case None =>
                          ...
                          Future failed (new TimeoutException(s"ref is empty in $w up to second timeout (not completed)"))
                    }
            }
          else
...
object ComputationBound {
  ...
  @volatile var globalDebug: Boolean = false
  ...
  def asyncCallback[A](source: (Try[A] => Unit) => Unit): ComputationBound[A] = {
    ...
    source(r => {
      if (globalDebug) then println(s"ComputationBound::asyncCallback listener called: $r")
      ref.set(Some(r))
  ...
  case class Deferred[A](ref: AtomicReference[Option[Try[A]]], optComputations: Option[ComputationBound[A]])
  ...
  def advanceDeferredQueueNoWait(deadlineMillis: Long): Boolean = {
    ...
    while (!secondQueue.isEmpty)
      ...
      if !(r eq null) then deferredQueue.add(r)
    nFinished > 0
  }
...
implicit object ComputationBoundAsyncMonad
    extends CpsAsyncMonad[ComputationBound]
    with CpsTryMonadInstanceContext[ComputationBound] {
